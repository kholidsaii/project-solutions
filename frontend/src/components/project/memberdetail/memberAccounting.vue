...
const userCOAs = ref<any[]>([]);

onMounted(async () => {
  try {
    const res = await api.get('/accounting/coas');
    // Menampilkan akun COA yang mengandung nama user (misal: "Piutang Karyawan - Kholid")
    const allCoas = res.data.data || res.data;
    userCOAs.value = allCoas.filter((c:any) => c.name.toLowerCase().includes(props.memberData.name?.toLowerCase()));
  ...
  <div class="space-y-6 animate-in fade-in duration-500">
    <div class="flex justify-between items-center bg-white rounded-[2.5rem] p-6 shadow-sm border border-slate-200">
      <div class="flex items-center gap-4">
         ...
         <div>
            ...
            <p class="text-[10px] text-slate-500 font-bold uppercase mt-0.5">Akun Akuntansi yang terhubung ke member ini</p>
         </div>
      </div>
    ...
    <div class="bg-white rounded-[3rem] border border-slate-200 shadow-sm overflow-hidden p-2">
      <table class="w-full text-left">
        ...
        <tbody>
          <tr v-if="userCOAs.length === 0">
            <td colspan="2" class="p-10 text-center text-slate-400 text-xs font-bold uppercase tracking-widest">Tidak ada akun COA personal / piutang terhubung.</td>
          </tr>
          <tr v-for="coa in userCOAs" :key="coa.id" class="border-t border-slate-50 hover:bg-slate-50">
            <td class="p-6">
