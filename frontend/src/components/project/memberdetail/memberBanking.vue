...
// Hanya memunculkan rekening yang tercatat atas nama member ini (atau rekening personal tanpa PT)
const personalBanks = computed(() => {
  return dbBanks.value.filter(b => b.user_id === props.memberData.id || (!b.pt_id && b.account_name.toLowerCase().includes(props.memberData.name?.toLowerCase())));
});
  ...
  <div class="animate-in fade-in duration-500 space-y-6">
    <div class="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h3 class="text-sm font-black text-slate-800 uppercase tracking-widest">Personal Bank Accounts</h3>
          <p class="text-[10px] text-slate-500 font-bold uppercase mt-0.5">Informasi rekening pribadi (Payroll)</p>
        </div>
      </div>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="bank in personalBanks" :key="bank.id" class="bg-white border border-slate-200 rounded-[2rem] p-6 relative overflow-hidden shadow-sm">
        ...
        <div class="flex items-start justify-between ml-2">
          ...
          <span class="px-3 py-1 rounded-full text-[9px] font-black text-slate-700 bg-yellow-300 uppercase shadow-sm"><i class="fas fa-user-tie mr-1"></i> Personal</span>
        </div>
      ...
      <div v-if="personalBanks.length === 0" class="col-span-full py-20 text-center bg-white border-2 border-dashed border-slate-200 rounded-[2rem]">
