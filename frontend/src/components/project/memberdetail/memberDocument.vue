...
// Mengambil data member dari parent (MemberDetail.vue)
const props = defineProps(['memberData']);

// Menarik data documents bawaan dari payload memberData
// Pastikan backend (API /users/{id}) menyertakan relasi 'documents'
const documents = computed(() => props.memberData?.documents || []);
  ...
  <div class="animate-in fade-in duration-500 space-y-6 w-full">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-slate-200 w-full">
      <div class="flex items-center gap-4 min-w-0">
        ...
        <div class="min-w-0">
          <h3 class="text-sm font-black text-slate-800 uppercase tracking-widest truncate">Personal Documents</h3>
          <p class="text-[10px] text-slate-500 font-bold uppercase mt-0.5 truncate">Arsip dan Lampiran milik {{ memberData?.name || 'Member' }}</p>
        </div>
      </div>
        ...
        <table class="w-full text-left min-w-[700px]">
          <thead class="bg-slate-50 border-b border-slate-100">
            <tr>
              ...
              <th class="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Uploaded By / Keterangan</th>
          ...
          <tbody class="divide-y divide-slate-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center gap-2">
                  <div class="w-7 h-7 shrink-0 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center text-[10px] font-black border border-indigo-100">
                    {{ (doc.uploader_name || 'H').substring(0,2).toUpperCase() }}
                  </div>
                  <span class="text-[10px] font-bold text-slate-600 capitalize truncate max-w-[120px]">{{ doc.uploader_name || 'HR/System' }}</span>
                </div>
              </td>
            ...
            <tr v-if="documents.length === 0">
              <td colspan="4" class="py-24 text-center">
                ...
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Belum Ada Dokumen Personal</p>
              </td>
            </tr>
          </tbody>
        </table>
