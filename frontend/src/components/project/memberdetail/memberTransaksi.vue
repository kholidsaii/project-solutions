...
onMounted(async () => {
  try {
    // API memfilter transaksi berdasarkan user_id (reimbursement, gaji, kasbon)
    const res = await api.get('/finance/transactions', { params: { user_id: props.memberData.id } });
    userTransactions.value = res.data;
  ...
  <div class="space-y-6 animate-in fade-in duration-500">
    <div class="flex justify-between items-center bg-white rounded-[2.5rem] p-6 shadow-sm border border-slate-200">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h3 class="text-sm font-black text-slate-800 uppercase">Personal Cashbook</h3>
          <p class="text-[10px] text-slate-500 font-bold uppercase mt-0.5">Reimbursement, Kasbon & Pembayaran</p>
        </div>
      </div>
    ...
    <table class="w-full text-left bg-white border border-slate-100 rounded-[2rem] overflow-hidden shadow-sm">
      ...
      <tbody>
        ...
        <tr v-for="trx in userTransactions" :key="trx.id" class="border-b border-slate-50 text-[11px] font-bold text-slate-600 hover:bg-slate-50 transition-colors">
          <td class="p-6">
