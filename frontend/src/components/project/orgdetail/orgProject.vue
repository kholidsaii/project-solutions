...
onMounted(async () => {
  try {
    ...
    // Filter proyek yang di dalamnya terdapat PT ini
    orgProjects.value = allProjects.filter((p:any) => 
      p.companies && p.companies.some((c:any) => c.id === props.companyData.id)
    );
  } catch (e) {
    console.error(e);
  } finally {
  ...
  <div class="space-y-6 animate-in fade-in duration-500 w-full">
    ...
    <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
       <div class="flex items-center gap-4">
          ...
          <div>
            <h3 class="text-sm font-black text-slate-800 uppercase tracking-widest">Corporate Projects</h3>
            <p class="text-[10px] text-slate-500 font-bold uppercase mt-0.5">Daftar project yang dikerjakan oleh {{ companyData?.name }}</p>
          </div>
       </div>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="project in orgProjects" :key="project.id" @click="goToProject(project.id)" class="bg-white border border-slate-200 rounded-[2rem] p-6 shadow-sm hover:border-indigo-300 hover:shadow-md transition-all cursor-pointer group">
      ...
      <div v-if="orgProjects.length === 0" class="col-span-full py-20 text-center border-2 border-dashed border-slate-200 rounded-[2rem]">
         <i class="fas fa-building text-4xl text-slate-200 mb-4"></i>
         <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Perusahaan ini belum ditugaskan di project manapun.</p>
      </div>
    </div>
  </div>
</template>